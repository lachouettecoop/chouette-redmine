  ...
  module Helper
    ...
    def replace_editor_script(field_id)
      ...
      (function() {
        ...
        var editor_options = #{RedmineCkeditor.options(@project).to_json};
        if (window.location.href.indexOf("/wiki/") > 0) {
            editor_options.toolbarCanCollapse = false;
            editor_options.toolbarLocation = 'top';
        }
        var editor = CKEDITOR.replace(textarea, editor_options);
